...
// 관련된 테스트의 내용을 묶어주는 역활
describe("block chain 동작 테스트", () => {
    // 생성할 블록의 갯수
    const CRAETE_BLOCK_NUM = 100000;
    ...
    beforeEach(() => {
        blockchain = new BlockChain(); // 제네시스 블록이 하나 생성되어 있는 chain 인스턴스
    })

    test(`블록 생성 테스트 ${CRAETE_BLOCK_NUM}개 체인 추가 검증`, () => {
        for (let i = 1; i < CRAETE_BLOCK_NUM; i++) {
