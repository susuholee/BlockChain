...
contract Proxy {
    ...
    // 슬롯의 영역은 CA로 저장하고 사용할 것

    // 프록시 컨트랙트 배포자 슬롯에 저장
    // 주소는 음수로 떨어질 수 없으니 그래서 - 1로 체크
    ...
    // setImpl() 슬롯에 값을 저장할 함수
    function setImpl(address _CA) external {
        // Slot 슬롯의 영역에서
        // Address 해시주소에 접근해서 참조
        // getAddressSlot () 주소를 참조해서 그 주소에 할당/조회 
    ...
    function delegate(address impl) private {
        assembly {
            ...
            switch result
            case 0 {
                revert(0, returndatasize()) // 오류 발생하면 오류 출력, 내용은 대리 호출로 반환
            }
            default {
...
// Slot
// 데이터의 저장을 슬롯에 할 수 있는 라이브러리
// library : 기능을 작성할 내용을 모듈화
// library 컨트랙트에서 재사용성이 높은 기능을 정리할 수 잇는 모듈화
library Slot {
    struct AddressSlot {
        address value;
    ...
    // pure를 사용해서 상태변수 접근을 못한다
    // 내부에서만 호출
    function getAddressSlot (bytes32 _slotAddress) internal pure returns(AddressSlot storage pointer) {
        assembly {
            // 주소 참조
            // 메모리의 주소를 반환하기 위해서 메모리의 주소를 참조
            // storage 스토리지 안에  메모리 영역에 접근 주소에 _slotAddress 값을 주소에 저장
