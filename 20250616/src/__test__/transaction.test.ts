...
describe("트랜잭션 검증", () => {
    ...
    test("트랜잭션 처리", () => {
        ...
        // UTXO 처리
        // 이전 처리과정 중에 트랜잭션 내용이 빠져있다
        const block = blockChain.addBlock([JSON.stringify(mempool.transaction[0])]);

        utxoLedger.updateTransaction(mempool.transaction[0]);
        mempool.removeTransaction(mempool.transaction[0].id);
...
// ts-node -> 노드 환경에서 메모리상에 컴파일된 코드를 잠시 적제하고 노드 런타임환경에서 실행하여 코드를 테스트한다
// jest -> ts-node와 같이 node의 런타임환경에서 테스트 코드를 실행
