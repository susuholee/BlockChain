export interface TxInput {
    txId : string; // 트랜잭션의 ID, 트랜잭션의 식별자  (거래소에 트랜잭션 ID를 신청하면)
...
// 코인 베이스 트랜잭션
// 코인 베이스 : 새로운 코인을 처음으로 만들어내는 거래


export interface TxOutput {
    address : string; // 주소 : 비트코인에서 주소 개념, 이더리움 이후에 계정이라는 용어가 유명해지면서 계정이란 뜻을 가짐
    amount : number;
}
